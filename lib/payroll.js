...
export function computePayroll(input, rates) {
  ...
  if (allowancesC > 0) {
    warnings.push({
      ...
      message: 'Non-taxable allowances are assumed to be within their de minimis ceilings. Anything above a ceiling is taxable, and counts against the ₱90,000 yearly limit for 13th month pay and other benefits.',
    });
  }
  if (taxAllowC > 0 && contributionBase === 'basic') {
    warnings.push({
      code: 'contribution_base',
      message: 'SSS, PhilHealth and Pag-IBIG are computed on basic pay only. Some employers include fixed allowances in the SSS base, which would raise the contribution slightly.',
    });
  }
